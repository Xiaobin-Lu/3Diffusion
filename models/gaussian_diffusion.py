...
class GaussianDiffusion:
    ...
    def p_mean_variance(
            self, model, x, hq, threeD, t, clip_denoised=True, denoised_fn=None, model_kwargs=None
    ):
        """
        ...
        B, C = x.shape[:2]
        assert t.shape == (B,)
        model_output = model(th.cat([x, threeD, threeD], dim=1), self._scale_timesteps(t), **model_kwargs)
    ...
    # 计算训练的loss，有三种类型的target（PREVIOUS_X、START_X、EPSILON）
