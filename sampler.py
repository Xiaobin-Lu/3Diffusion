...
class BaseSampler:
    ...
    def build_model(self):
        ...
        print("self.configs.model.ckpt_path",self.configs.model.ckpt_path)
...
class DifIRSampler(BaseSampler):
    def load_model(self, model, ckpt_path=None):
        if not ckpt_path is None:
            if self.rank == 0 and self.display:
                print(f'Loading from {ckpt_path}...')
            ckpt = torch.load(ckpt_path, map_location=f"cuda:{self.rank}")
            util_net.reload_model2(model, ckpt,True)
            if self.rank == 0 and self.display:
                print('Loaded Done')
    ...
    def sample_func_ir_aligned(
        ...
        Input:
            ...
            model_kwargs_ir: additional parameters for ema0999_model_67000 model
        Output:
        ...
        # print("change hq and threeD")
        assert yt.shape[-1] == self.configs.im_size and yt.shape[-2] == self.configs.im_size
        print("self.configs.diffusion.params.timestep_respacing:",self.configs.diffusion.params.timestep_respacing)
        if 'ddim' in self.configs.diffusion.params.timestep_respacing:
    ...
    def sample_func_bfr_unaligned(
        ...
        Input:
            ...
            bs: batch size for face ema0999_model_67000
        ...
        Output:
            ...
            4.17: list, h x w x c, numpy array, uint8, BGR
